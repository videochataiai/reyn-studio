...
5. **The day-one essentials audit finds Reyn strongest exactly where the PRD invested** — geometry preflight, run gating, immutable lineage, provenance-carrying export — **and weakest in the interactive layer around the solve:** no run cancellation, no determinate progress, no horizon playback, no 3D probe, no viewport image export, no engineering-case report (benchmark reports exist; engineering reports do not).

6. **Two of the highest-credibility gaps are cheap because the engine already supports them.** The engine caches the solver warmup per voxel mask so "horizon changes are one model pass" (`src/engine.rs:142-144`) — horizon scrubbing/playback is nearly free. And `run_external_flow` already threads a `request_id` through the protocol (`src/app.rs:913-921`) — cancellation has a natural hook.
...
| **Cancellation** | MISSING | No cancel path anywhere (grep: only dialog Cancel buttons); a hung engine call leaves `pending` until timeout/restart |
...
**R2 — Run progress + cancellation. (P0)**
The request already carries `request_id` (`src/app.rs:913-921`). Add `engine::Cmd::CancelRequest { request_id }` and have the Python sidecar check a cancellation flag between warmup, per-step model passes, and derivation, replying with a `cancelled` stop reason recorded in the run manifest (stop reason is already a manifest field, PRD §5). UI: elapsed time from `PendingCadRun.started_at` (`src/app.rs:57-61`) plus a stage string ("warmup → predict step k/H → derive loads") in the status-bar center slot reserved for exactly this (`src/app.rs:3375-3450`), and a quiet Cancel affordance next to the Run button. A cancelled attempt is still an immutable run record with its stop reason — never silently discarded.
